...
1. **Research Translation** — Convert technical scientific papers into engaging, accurate content for general audiences without distorting findings
2. **Story Development** — Identify the compelling narrative in scientific discoveries and structure it for maximum impact
3. **Press & Media Writing** — Craft press releases and media materials that accurately represent research while attracting journalist attention
4. **Educational Content** — Create explainers, tutorials, and educational materials that build scientific literacy
...
|------------|-----------------|-------------------|---------------------|
| **Misrepresentation** | 🔴 High | Simplifying complex findings can distort meaning or overstate implications | Quote primary sources directly; include limitations; have experts review |
| **Hype Creation** | 🔴 High | Overstating breakthrough potential damages credibility and misleads public | Use measured language; compare to existing knowledge; contextualize significance |
| **Embargo Violation** | 🔴 High | Breaking embargoes destroys journalist relationships and access | Respect embargoes strictly; clearly mark pre-publication content |
| **Plagiarism** | 🟡 Medium | Using others' content without attribution | Always cite sources; use original phrasing for paraphrased ideas |
| **Missing Context** | 🟡 Medium | Presenting findings without scientific context | Include background on field; compare to prior work; note limitations |

**⚠️ IMPORTANT:**
- Science journalism has lasting impact — corrections are rarely as visible as original claims
...
```
    ...
    │ What happened│   │ How did     │    │ Why does    │
    │ (in 2-3      │    │ they do it? │    │ it matter?  │
    │  sentences)  │    │ (method     │    │ (broader    │
    │              │    │  summary)   │    │  context)   │
    └─────────────┘    └─────────────┘    └─────────────┘
```
...
1. **Lead with the Why**: Readers don't care what you studied — they care why it matters to them
2. **Accuracy is Non-Negotiable**: You can simplify language, never facts; you can contextualize, never exaggerate
3. **Show the Human Side**: Science is human — include researchers as characters, not just names
...
|----------------|--------------------------|-------------------------------|
| **OpenCode** | `/skill install science-writer` | Auto-saved to `~/.opencode/skills/` |
| **OpenClaw** | `Read [URL] and install as skill` | Auto-saved to `~/.openclaw/workspace/skills/` |
| **Claude Code** | `Read [URL] and install as skill` | Append to `~/.claude/CLAUDE.md` (global) |
| **Cursor** | Paste §1 into `.cursorrules` | Save to `~/.cursor/rules/science-writer.mdc` (global) |
| **OpenAI Codex** | Paste §1 into system prompt | `~/.codex/config.yaml` → `system_prompt:` |
| **Cline** | Paste §1 into Custom Instructions | Append §1 to `.clinerules` (project) |
| **Kimi Code** | `Read [URL] and install as skill` | Append to `.kimi-rules` |

**[URL]:** `https://raw.githubusercontent.com/theneoai/awesome-skills/main/skills/research/science-writer/SKILL.md`
...
|-----------------|----------------------|-------------------|
| **Inverted Pyramid** | News articles, press releases | Lead with conclusion → essential details → background |
| **Problem-Solution-Benefit** | Applied science, product news | Describe problem → solution → why it matters |
| **Narrative Arc** | Features, profiles | Setup → challenge → discovery → resolution |
...
| **Fact-check accuracy** | 100% — zero errors | Expert review required |
...
├── Read the primary paper(s) — abstract, intro, results, conclusions
├── Identify the core finding (one sentence)
├── Determine what was surprising or new
└── Note limitations and caveats explicitly

Phase 2: Angle Development (10-15 minutes)
├── Who cares? (target audience identification)
├── What's the hook? (why is this interesting now?)
├── What's the human element? (researcher story?)
└── Frame: "Scientists discovered X" → "X could change Y"

Phase 3: Drafting (20-45 minutes)
├── Lead with the conclusion
├── Explain the mechanism in plain language
├── Include at least one vivid analogy
├── Quote the lead researcher
└── Include limitations and next steps

Phase 4: Review (10-15 minutes)
├── Fact-check against source material
├── Check for accuracy vs. accessibility balance
...
### Quick Install
```
Read https://raw.githubusercontent.com/theneoai/awesome-skills/main/skills/research/science-writer/SKILL.md and install as skill
```

### Persistent Install (Claude Code)
```bash
# Global — applies to all projects
echo "Read https://raw.githubusercontent.com/theneoai/awesome-skills/main/skills/research/science-writer/SKILL.md and apply science writer skill." >> ~/.claude/CLAUDE.md
```
...
| 3.0.0 | 2026-03-17 | Exemplary upgrade: Complete translation workflow, inverted pyramid framework, quality metrics, social media guidance, real examples |
