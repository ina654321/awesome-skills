...
### Scenario 1: Reducing Store Shrink from 2.1% to <1.0%

**Context:** Specialty apparel store, $2.5M annual revenue. Shrink at 2.1% = $52,500 loss. Industry average: 1.3%.

**Root Cause Analysis:**
```
Shrink Breakdown:
  • External (shoplifting): 45% = $23,625 — high-theft accessories (scarves, jewelry)
  • Internal (employee): 30% = $15,750 — excessive voids at register
  • Process (receiving): 25% = $13,125 — delivery shortages unverified

Action Plan:
  Week 1-2: Install EAS pedestals at entrance; tag all accessories
  Week 3-4: Implement dual-authorization for voids >$25
  Week 5-6: Triple-count all deliveries with photo documentation
  Week 7-8: Associate training on shoplifting prevention de-escalation
  Week 9-12: Measure shrink reduction; adjust strategy

Result Target: 2.1% → 1.2% = $22,500 recovered annually
```

### Scenario 2: Omnichannel Launch — BOPIS in 30 Days

**Context:** Brick-and-mortar electronics store, 3 locations. Adding BOPIS for 500 SKUs.

**Implementation Checklist:**
```
Week 1: Technology Setup
  □ Select POS integration (NetSuite + Shopify POS)
  □ Configure inventory sync (real-time, 15-min intervals)
  □ Designate pickup counter area with signage

Week 2: Process Design
  □ Write BOPIS fulfillment SOP (pick, pack, stage, notify)
  □ Train all associates on new workflow
  □ Test 10 pilot orders internal

Week 3: Marketing & Launch
  □ Update website with BOPIS option
  □ In-store signage promoting new service
  □ Launch: 500 SKUs available

Week 4: Optimization
  □ Monitor: fulfillment time, pickup rate, customer feedback
  □ Adjust: staffing during peak pickup hours
  □ Expand: add 200 more SKUs based on sell-through

Metrics Target:
  • Fulfillment time: <2 hrs (target: 90 min)
  • Pickup rate: 75%+ of orders picked up
  • Customer satisfaction: 4.5+ stars
```

### Scenario 3: Inventory Accuracy Crisis — System Says 500, Floor Has 200

**Context:** Quarterly inventory audit reveals 300-unit discrepancy in electronics category ($45,000 retail value). System shows 500 units; physical count shows 200.

**Resolution Process:**
```
Step 1: Verify System Data
  • Review receiving history for electronics category (past 90 days)
  • Check for unrecorded damages, returns, or transfers
  • Result: System appears accurate

Step 2: Floor Investigation
  • Physical walk of entire store with scanner
  • Check: behind displays, backroom, fitting rooms, damaged goods hold
  • Result: Found 180 units mislocated in other categories

Step 3: Resolution
  • Move 180 units to correct location: +$27,000 recovered
  • Remaining 120 units unaccounted: $18,000 shrink write-off
  • Root cause: Associates not scanning when moving product

Step 4: Prevention
  • Implement "move scan" requirement (scan out + scan in)
  • Weekly location accuracy spot-checks
  • Add to associate training: inventory accuracy importance
```
