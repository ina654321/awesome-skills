...
-- Or use Liquid Clustering (DBR 13.3+)
...
-- Use predictive optimization for auto-VACUUM
ALTER TABLE table_name SET TBLPROPERTIES (
...
## Anti-Pattern 13: Vector Search Without Sync Monitoring

### ❌ Wrong
```python
# Assume vector index is always current
results = client.similarity_search(
    index_name="docs_index",
    query_text=query
)
# No check if index is stale
```

### ✅ Right
```python
# Check index sync status before querying
index_info = client.get_index("docs_endpoint", "docs_index")
last_sync = index_info["delta_sync_index_spec"]["last_synced_version"]
current_version = spark.table("catalog.docs.chunks").selectExpr("max(_commit_version)").collect()[0][0]

if last_sync < current_version:
    # Trigger sync or use fallback
    client.sync_delta_index("docs_endpoint", "docs_index")
...
**Impact**: Stale embeddings cause RAG to return outdated or incorrect information.

---

## Anti-Pattern 14: LLM Without Governance

### ❌ Wrong
```python
# Direct LLM calls without controls
response = client.predict(
    endpoint="databricks-dbrx-instruct",
    inputs={"messages": [{"role": "user", "content": user_input}]}
)
# No content filtering, no PII detection
```

### ✅ Right
```python
# Use Mosaic AI Gateway for governance
from databricks.model_serving.gateway import GatewayClient

gateway = GatewayClient()

# Apply content filtering and PII detection
response = gateway.invoke(
    endpoint="gateway-prod",
    inputs={
...
**Impact**: Ungoverned LLM usage risks data leakage, PII exposure, and compliance violations.

---

## Anti-Pattern 15: Ignoring Model Serving Latency

### ❌ Wrong
```python
# Deploy without latency optimization
client.create_endpoint(
    name="high-latency-endpoint",
    config={
        "served_entities": [{
            "entity_name": "LargeModel",
            "entity_version": "1",
            "workload_size": "Small",  # Too small for traffic
            "scale_to_zero_enabled": True  # Cold start latency
        }]
    }
)
```
...
client.create_endpoint(
    name="optimized-endpoint",
    config={
        "served_entities": [{
            "entity_name": "LargeModel",
            "entity_version": "1",
            "workload_size": "Medium",
            "scale_to_zero_enabled": False,  # Keep warm for consistent latency
            "provisioned_throughput": 1000  # Provisioned capacity
        }],
        "route_optimized": True,  # Enable route optimization
        "auto_capture_config": {
...
WHERE endpoint_name = 'optimized-endpoint'
  AND timestamp > current_timestamp() - INTERVAL 1 HOUR
""").show()
```

**Impact**: Poor latency configuration leads to bad user experience and SLA violations.

---

## Anti-Pattern 16: Not Using Unity Catalog for AI Assets

### ❌ Wrong
```python
# Store models outside Unity Catalog
mlflow.set_registry_uri("databricks")
mlflow.sklearn.log_model(model, "model")  # Goes to workspace registry
```
...
mlflow.sklearn.log_model(
    model,
    artifact_path="model",
    registered_model_name="production.ml.fraud_detection"
)

# Benefit: Unified governance, lineage, access control
```

**Impact**: Workspace-level models lack enterprise governance and cross-workspace sharing.
